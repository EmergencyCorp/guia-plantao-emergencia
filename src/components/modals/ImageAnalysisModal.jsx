		...
		<div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
			<div className={`w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-white'}`}>
				<div className={`p-4 border-b flex justify-between items-center ${isDarkMode ? 'bg-blue-900/30 border-slate-800' : 'bg-blue-600 border-blue-700'}`}>
					<h3 className={`font-bold flex items-center gap-2 ${isDarkMode ? 'text-blue-300' : 'text-white'}`}><Camera size={24} /> IA Vision - Análise Clínica e de Imagem</h3>
				...
				<div className="p-6 overflow-y-auto flex-1">
					{!imageAnalysisResult ? (
						<div className="space-y-6">
                            ...
                            {/* --- ÁREA DE UPLOAD CORRIGIDA PARA MOBILE/FOTO --- */}
							<div onClick={triggerFileInput} className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-colors group cursor-pointer ${isDarkMode ? 'border-slate-700 bg-slate-800/50 hover:border-blue-500' : 'border-gray-300 bg-gray-50 hover:border-blue-500'}`}>
                                
                                {/* Input Real (Escondido) */}
								<input 
                                    ref={fileInputRef}
                                    type="file" 
                                    accept="image/*" // Aceita qualquer imagem (foto ou exame)
								...
								{selectedImage ? (
									<div className="relative h-64 w-full">
										...
										<div className="absolute bottom-2 right-2 bg-black/50 text-white px-3 py-1 rounded text-xs">Clique/Toque para trocar a imagem</div>
									</div>
								) : (
									<div className="space-y-2 pointer-events-none">
										...
										<h4 className={`font-bold ${isDarkMode ? 'text-slate-200' : 'text-slate-700'}`}>Arraste, clique ou use a câmera do celular</h4>
										<p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Suporta: ECG, Raio-X, Tomografia, **Lesões Cutâneas**, **Catarros**, **Fezes**, **Monitores**...</p>
									</div>
								)}
							</div>
							...
							<div>
								...
								<div className="relative">
									<input type="text" value={imageQuery} onChange={(e) => setImageQuery(e.target.value)} placeholder="Ex: Onde está a fratura? / Qual o ritmo deste ECG? / Qual a etiologia provável desta lesão?" className={`w-full pl-4 pr-12 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 font-medium ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500' : 'bg-white border-gray-200 text-slate-800'}`} />
							...
							<div className={`border p-3 rounded-lg flex gap-3 items-start ${isDarkMode ? 'bg-yellow-900/20 border-yellow-800/30' : 'bg-yellow-50 border-yellow-100'}`}>
								...
								<p className={`text-xs text-justify ${isDarkMode ? 'text-yellow-200' : 'text-yellow-800'}`}><strong>Atenção:</strong> As imagens são processadas em tempo real e <strong>deletadas imediatamente</strong> após fechar esta janela. Use com cautela, a IA não substitui o julgamento clínico.</p>
							</div>
						</div>
					) : (
