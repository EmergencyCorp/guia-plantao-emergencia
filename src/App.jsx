...
function EmergencyGuideAppContent() {
	...
	const handleAnalyzeImage = async () => {
		...
		try {
			const controller = new AbortController();
			const timeoutId = setTimeout(() => controller.abort(), 60000); // Maior timeout para análise de imagem
			...
			const response = await fetch('/api/generate', {
				...
				headers: headers,
				body: JSON.stringify({ 
					image: selectedImage, // Base64 com MimeType
					prompt: imageQuery,
					mode: 'image'
				}),
				signal: controller.signal
			...
			showError(`Falha na análise. ${error.message || 'Verifique o console para detalhes.'}`); 
			setImageAnalysisResult("Erro ao analisar imagem. Tente novamente.");
		} finally { 
	...
	const generateBedsideConduct = async () => {
		...
		try {
			...
			// Chamada real para a Serverless Function
			const response = await fetch('/api/generate', {
				method: 'POST', 
				headers: headers,
				body: JSON.stringify({ 
			...
			// A Serverless Function retorna o JSON já pronto
			const result = await response.json(); 
			setBedsideResult(result);

		} catch (error) { 
			console.error("Erro Bedside:", error);
			showError(`Erro ao processar. ${error.message || 'Verifique o console para detalhes.'}`);
			// Mock de contingência em caso de erro grave (opcional)
			setBedsideResult({ 
				hipoteses_diagnosticas: ["Falha de Comunicação"], 
				racional_clinico: "Erro na comunicação com a IA. Tente novamente.", 
				conduta_terapeutica: [{ tipo: "Erro", detalhe: "Tente novamente." }],
				solicitacao_exames: ["Nenhuma"],
				encaminhamentos: ["Nenhum"],
				cuidados_gerais: ["Nenhum"],
				orientacoes_paciente: ["Aguardar"]
			});
		} finally { 
